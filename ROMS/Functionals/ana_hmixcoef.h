...
#elif defined CIRCUMPOLAR
!
! Circumpolar Antarctic northern boundary sponge.
!
     fac=10.0_r8
# if defined UV_VIS2
     IF ((Mm(ng)+1-JendR).le.15) THEN
       DO i=IstrT,IendT
         DO j=MAX(JstrT,Mm(ng)+1-15),JendT
           cff=visc2(ng)+REAL(j-Mm(ng)-1+15,r8)*fac*visc2(ng)/15.0_r8
           MIXING(ng) % visc2_r(i,j)=cff
           MIXING(ng) % visc2_p(i,j)=cff
         END DO
       END DO
...
# elif defined UV_VIS4
     IF ((Mm(ng)+1-JendR).le.15) THEN
       DO i=IstrT,IendT
         DO j=MAX(JstrT,Mm(ng)+1-15),JendT
           cff=visc4(ng)+REAL(j-Mm(ng)-1+15,r8)*fac**2*visc4(ng)/15.0_r8
           MIXING(ng) % visc4_r(i,j)=cff
           MIXING(ng) % visc4_p(i,j)=cff
         END DO
       END DO
     END IF
# endif
     ...
     IF ((Mm(ng)+1-JendR).le.15) THEN
       DO itrc=1,NAT
         DO i=IstrT,IendT
           DO j=MAX(JstrT,Mm(ng)+1-15),JendT
             cff=tnu2(itrc,ng) +                                           &
     &           REAL(j-Mm(ng)-1+15,r8)*fac*tnu2(itrc,ng)/15.0_r8
             MIXING(ng) % diff2(i,j,itrc)=cff
           END DO
         END DO
       END DO
     ...
     IF ((Mm(ng)+1-JendR).le.15) THEN
       DO itrc=1,NAT
         DO i=IstrT,IendT
           DO j=MAX(JstrT,Mm(ng)+1-15),JendT
             cff=tnu4(itrc,ng) +                                           &
     &           REAL(j-Mm(ng)-1+15,r8)*fac**2*tnu4(itrc,ng)/15.0_r8
             MIXING(ng) % diff4(i,j,itrc)=cff
           END DO
         END DO
       END DO
     END IF
