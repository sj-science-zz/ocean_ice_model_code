...
!=======================================================================
      ...
      subroutine read_data_nc (flag, recd, yr, ixm, ixx, ixp, &
                            ...
                            field_loc, field_type, forcing_halo)
...
! Adapted by Alison McLaren, Met Office from read_data
      ...
      logical (kind=log_kind), optional, intent(in) :: &
         forcing_halo            ! if true, throw away edges of field
	                         ! (ROMS needs these edges, CICE doesn't)
...
!jd
            if (present(forcing_halo)) then
               call ice_read_nc & 
                    (fid, nrec, fieldname, field_data(:,:,arg,:), dbug, &
                    field_loc, field_type, forcing_halo=forcing_halo)
	    else
	       call ice_read_nc & 
                    (fid, nrec, fieldname, field_data(:,:,arg,:), dbug, &
                    field_loc, field_type)
...
! Compute coefficients for interpolating monthly data to current time step.
      ...
      ! make time cyclic
      !tt = mod(ftime/secday,dayyr) KA: This causes problems if you have
      ! leap years turned on as dayyr changes throughout the simulation!!
      tt = daycal(month) + mday + sec/secday
...
!jd END
      ...
      do i = ilo, ihi
        ...
        ! convert precipitation units to kg/m^2 s
	 frain(i,j) = frain(i,j) * precip_factor
         fsnow(i,j) = fsnow(i,j) * precip_factor
      enddo                     ! i
      enddo                     ! j

      ! determine whether precip is rain or snow
      ! HadGEM and ECMWF forcing provides separate snowfall and rainfall rather 
      ...
      ! Subtract snow from rain in ECMWF so it isn't counted twice
      if (trim(atm_data_type) == 'ecmwf') then

        do j = jlo, jhi
	do i = ilo, ihi
	   frain(i,j) = max(frain(i,j)-fsnow(i,j), c0)
	enddo
	enddo
...
! authors: Keguang Wang, met.no
      ...
      rain_file = &
           trim(atm_data_dir)//'FC_1996_subdaily.nc'
      call file_year(rain_file,yr)

      snow_file = rain_file

      tair_file = &
          trim(atm_data_dir)//'AN_1996_subdaily.nc'
      call file_year(tair_file,yr)

      uwind_file = tair_file
      vwind_file = tair_file
      humid_file = tair_file
      rhoa_file  = tair_file
      flw_file   = tair_file      
...
!ka I have adjusted the time axis in my NetCDF forcing files by 12 hours to
!   make things easier for ROMS, but this doesn't change any of the code here
...
!jd
         ...
         call ice_read_nc (fid, recnum, fieldname, frain, dbug, &
...
!         if (my_task == master_task) write(nu_diag,*) 'ecmwf_data: precip_factor ', precip_factor
	 ...
	 call ice_open_nc(snow_file,fid)
	 call ice_read_nc (fid, recnum, fieldname, fsnow, dbug, &
	      field_loc_center, field_type_scalar, forcing_halo=.true.)
	 call ice_close_nc(fid)

         ! Assume same units as rain; convert as necessary
	 do iblk = 1, nblocks
...
!jd
      ...
      call read_data_nc (read6, 0, fyear, ixm, ixx, ixp, &
                   maxrec, uwind_file, fieldname, uatm_data, &
            field_loc_center, field_type_vector, forcing_halo=.true.)
      fieldname='Vwind'
      call read_data_nc (read6, 0, fyear, ixm, ixx, ixp, &
                   maxrec, vwind_file, fieldname, vatm_data, &
            field_loc_center, field_type_vector, forcing_halo=.true.)
